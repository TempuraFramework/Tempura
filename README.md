...
### Transforms in `c.transforms`

****NOTE****: I used claude to auto-generate the descriptions below and haven't cleaned them up yet, and this is subject to a lot of changes

  This section summarizes the transforms implemented via the Tempura transforms pipeline (in `tempura/cozy/Transforms.scala`, `tempura/cozy/AutoRegister.scala` and `tempura/cozy/Registry.scala`, which is auto-generated by sbt via `build.sbt`)
  In general, a transform takes in a $k$-ary tuple (modeled as a sequence / list in Clojure) and returns a $k$-ary tuple, or an error message.
  
  To call these transforms via: (c.transforms/transform-name args...)

####  I/O Transforms

 *** path-of ***
  ```
  Convert string path to Java Path object
  Call: (c.transforms/path-of "path/to/file")
  Returns: Path object
  Example: (c.transforms/path-of "/Users/foo/model.vmt")
```
  *** read-string ***
```
  Read file contents as string
  Call: (c.transforms/read-string path-object)
  Returns: String containing file contents
  Pipeline: Takes Path → returns String
  Example:
  (def path (c.transforms/path-of "model.vmt"))
  (def content (c.transforms/read-string path))
```

 #### Expression Parsing Transforms

 *** parse-sexpr***
```
  Parse string into S-expression AST (ParseTree list)
  Call: (c.transforms/parse-sexpr string-content)
  Returns: List[ParseTree] - abstract syntax tree
  Example:
  (c.transforms/parse-sexpr "(declare-const x Int)")
  ; => List of ParseTree nodes
 ```
 *** parse-smtlib ***
```
  Parse SMT-LIB S-expression into Core IR expression
  Call: (c.transforms/parse-smtlib type-env interp-env parse-tree)
  Arguments:
  - type-env: TypeEnv (from (c.solver/type-env))
  - interp-env: InterpEnv (from (c.solver/interp-env))
  - parse-tree: Single ParseTree node
  Returns: Core.BoxedExpr
  Example:
  (def te (c.solver/type-env))
  (def ie (c.solver/interp-env))
  (c/var 'x 'Int)
  (def parsed (c.transforms/parse-sexpr "(> x 0)"))
  (c.transforms/parse-smtlib te ie (first parsed))
```
 *** parse-vmt ***
```
  Parse VMT command list into TransitionSystem
  Call: (c.transforms/parse-vmt type-env interp-env parse-tree-list)
  Arguments:
  - type-env: TypeEnv
  - interp-env: InterpEnv
  - parse-tree-list: List of ParseTree nodes
  Returns: TransitionSystem
```

***  read-sexpr ***
```
  Read file and parse to S-expressions (pipeline: path → string → parse)
  Call: (c.transforms/read-sexpr "filepath")
  Returns: List[ParseTree]
  Pipeline: PathOf → StringReader → StringToSExprParser
  Example:
  (def sexpr-ast (c.transforms/read-sexpr "model.vmt"))
```
*** read-vmt ***
```
  Read VMT file and parse into TransitionSystem
  Call: (c.transforms/read-vmt "filepath")
  Returns: TransitionSystem
  Pipeline: PathOf → StringReader → StringToSExprParser → VMTParser
  Example:
  (def ts (c.transforms/read-vmt "/path/to/model.vmt"))
```
***  read-tdl ***
```
  Read TDL (Tempura Definition Language) file into TransitionSystem
  Call: (c.transforms/read-tdl "filepath")
  Returns: TransitionSystem
  Pipeline: PathOf → StringReader → StringToSExprParser → TDLParser
  Note: TDL is Tempura's native format with extensions like state-var, transition-var, theory-axiom
```

***  print-smtlib ***
```
  Convert Core IR expression to SMT-LIB2 string
  Call: (c.transforms/print-smtlib type-env interp-env expr)
  Arguments:
  - type-env: TypeEnv
  - interp-env: InterpEnv
  - expr: Core.BoxedExpr
  Returns: String in SMT-LIB2 format
  Example:
  (def te (c.solver/type-env))
  (def ie (c.solver/interp-env))
  (def expr (c/expr '(and x y)))
  (c.transforms/print-smtlib te ie expr)
  ; => "(and x y)"
```
 *** print-tdl***
```
  Convert TransitionSystem to TDL format string
  Call: (c.transforms/print-tdl type-env interp-env transition-system)
  Arguments:
  - type-env: TypeEnv
  - interp-env: InterpEnv
  - transition-system: TransitionSystem
  Returns: String in TDL format with all sorts, variables, transitions, properties
  Output includes: Sorts, state variables, transition variables, init, transitions, properties, assumptions, liveness specs
```
 
### Transforms to do with Model Checking

 *** mine-index-term ***
```
  Synthesize ranking index term 
  Call: (c.transforms/mine-index-term solver-env transition-system state-graph)
  Arguments:
  - solver-env: SolverEnvironment (from create-solver)
  - transition-system: TransitionSystem
  - state-graph: StateGraph (from bounded model checking)
  Returns: Core.BoxedExpr - candidate index term
```
  *** synth-rel-ranking ***
  ```TODO ***
  ```
