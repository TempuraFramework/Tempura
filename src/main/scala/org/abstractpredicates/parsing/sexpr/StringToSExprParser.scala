...
object StringToSExprParser extends BasicTransform[String, List[ParseTree]](using summon[ClassTag[String]], summon[ClassTag[List[ParseTree]]]) {
  ...
  override def apply(input: String): Either[String, List[ParseTree]] = {
    this.sexprs.parseAll(input.strip()) match {
