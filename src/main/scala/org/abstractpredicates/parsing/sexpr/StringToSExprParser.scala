...
import org.abstractpredicates.parsing.sexpr.{ParseTree, ParseValue}


object StringToSExprParser extends TempuraTransform[String, List[ParseTree]] {
  ...
  override def apply(typeEnv: Core.TypeEnv, interpEnv: Core.InterpEnv)(input: String): Either[String, List[ParseTree]] = {
    this.sexprs.parseAll(input) match {
      case Left(error) => Left(error.toString)
      case Right(l) => Right(l)
    }
  }
  
  def apply(input: String): Either[String, List[ParseTree]] =
    apply(Core.emptyTypeEnv, Core.emptyInterpEnv)(input)
}
