...
object Core {
  ...
  enum SortValue[S <: Sort[S]](val sort: S) {
    ...
    override def hashCode(): Int =
      this match {
        case BoolValue(b) => b.hashCode()
        case NumericValue(n) => n.hashCode()
        case ArrayValue(_, s) => s.hashCode()
        case FunctionValue(_) => 1
        case FiniteUniverseValue(i, s) => i.hashCode() ^ s.hashCode()
        case UnInterpretedValue(n, s) => n.hashCode() ^ s.hashCode()
        case AliasSortValue(n, s) => n.hashCode() ^ s.hashCode()
        case DatatypeValue(ic, s) => ic.hashCode() ^ s.hashCode()
      }

    override def equals(obj: Any): Boolean = {
      (this, obj) match {
        case (NumericValue(n1), NumericValue(n2)) => n1 == n2
        case (BoolValue(b1), BoolValue(b2)) => b1 == b2
        case (ArrayValue(v1, s1), ArrayValue(v2, s2)) => v1 == v2 && s1 == s2
        case (FunctionValue(d1), FunctionValue(d2)) => d1 == d2
        case (FiniteUniverseValue(i1, s1), FiniteUniverseValue(i2, s2)) => i1 == i2 && s1 == s2
        case (UnInterpretedValue(n1, s1), UnInterpretedValue(n2, s2)) => n1 == n2 && s1 == s2
        case (AliasSortValue(a1, s1), AliasSortValue(a2, s2)) => a1 == a2 && s1 == s2
        case (DatatypeValue(ic1, s1), DatatypeValue(ic2, s2)) => ic1 == ic2 && s1 == s2
        case _ => false
      }
    }
  }
  ...
  final def canonicalNameOfValue[S <: Sort[S]](sortValue: SortValue[S]): String = {
    sortValue match {
      ...
      case SortValue.ArrayValue(a, s) => s"v_arr_${a}"
      case SortValue.FunctionValue(d) => s"v_func_${d.name}"
      case SortValue.FiniteUniverseValue(idx, s) => s"v_fd_$s.name_${idx}"
      case SortValue.UnInterpretedValue(n, s) => s"v_unk_${n}_${s.name}"
      case SortValue.AliasSortValue(n, _) => unsupported("no name for alias sort value")
      case SortValue.DatatypeValue(ic, s) => s"v_dt_${ic.constructor.name}_${s.name}"
    }
  }
    ...
    def apply(interpEnv: InterpEnv, transform: ExprTransformer): InterpEnv = {
      ...
      interpEnv.foreach(
        (name, entry) =>
          ...
          newInterpEnv.add(name, newEntry)
      )
      newInterpEnv
    }
  }
