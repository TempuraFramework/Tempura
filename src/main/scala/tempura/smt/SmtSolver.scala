...
object SmtSolver {
  ...
  trait Solver[LT, LVD](typeEnv: Core.TypeEnv, interp: Core.InterpEnv) {
    ...
    def solveOnce[S <: Core.Sort[S]](fmla: Core.Expr[S]): Result = {
      push()
      val res = checkSat()
      pop()
      res
    ...
    def allSat(vocab: Set[(String, Core.BoxedSort)]): List[Model[LT, LVD]] = {
      partialAllSat(vocab, -1)
    }

    def partialAllSat(vocab: Set[(String, Core.BoxedSort)], numModels: Int): List[Model[LT, LVD]] = {
      var models: List[Model[LT, LVD]] = List()
      ...
      if numModels == 0 then return List()
        ...
        while (!stop) {
          ...
          if checkSat() != Result.SAT then {
            ...
            case Some(m) =>
              ...
              models = m :: models
              if numModels >= 0 && models.size >= numModels then
                stop = true
            case None =>
              stop = true
