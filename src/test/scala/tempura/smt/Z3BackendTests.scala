...
class Z3BackendTests extends AnyFunSuite {
  ...
  */
  ...
  test("uninterpreted sorts handling") {
    val (typeEnv, interpEnv) = (Core.emptyTypeEnv, Core.emptyInterpEnv)
    val solver = Z3Solver.Z3Solver(typeEnv, interpEnv)
    val z3 = solver.getContext
    val sortU = typeEnv |- Core.UnInterpretedSort("U", 0)
    val x = interpEnv |- ("x", sortU)
    val y = interpEnv |- ("y", sortU)
    solver.push()
    solver.add(List(Core.mkNot(Core.mkEq(x, y))))
    assert(solver.checkSat() == Result.SAT)
    val m = solver.getModel.get
    solver.pop()
    println("model: " + m.model.toString)
    println("model getSortUniverse: " + m.model.getSortUniverse(solver.lowerSort(sortU)).mkString("Array(", ", ", ")"))
    val universe = m.model.getSortUniverse(solver.lowerSort(sortU)).map(x => solver.liftTerm(x))
    val universe1 = m.model.getSortUniverse(solver.lowerSort(sortU))
    println("translated universe:" + universe.mkString(", "))
    val universeConstantLowered = universe1(0)
    val universeConstant = universe(0)
    solver.push()
    solver.addTerms(z3.mkEq(solver.lower(universeConstant), universeConstantLowered).asInstanceOf[solver.LoweredTerm])
    assert(solver.checkSat() == Result.SAT)
    println(solver.getModel.get)
