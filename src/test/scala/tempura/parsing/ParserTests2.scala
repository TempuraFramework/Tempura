...
import tempura.parsing.io.{PathOf, SexprReader, StringReader, TDLReader, VMTReader}
...
class ParserTests2 extends AnyFunSuite {
  ...
  test("read entire VMT file string to SExpr") {
    StringToSExprParser(Tuple1(test0)) match {
      case Right(Tuple1(nodes)) =>
        println(s"successfully parsed: ${nodes}")
        assert(true)
      case Left(err) =>
  ...
  test("read entire VMT file string and compare it with test0") {
    val pipeline = PathOf *: StringReader *: EmptyTuple
    Transforms.compose(Tuple1("examples/ranking_infer1_trimmed.vmt"), pipeline) match {
      case Right(Tuple1(inputString)) =>
  ...
  test("read entire VMT file string and parse it into Sexpr") {
    ...
    Transforms.compose(Tuple1("examples/ranking_infer1_trimmed.vmt"), pipeline) match {
      case Right(Tuple1(inputSExpr)) =>
        println("Read input: " + inputSExpr.toString)
        assert(true)
      case Left(error) =>
        println(s"got error: ${error}")
        assert(false)
  ...
  test("read entire VMT file string and parse it into PreTransitionSystem") {
    VMTReader(Tuple1("examples/ranking_infer1_trimmed.vmt"))  match {
      case Right(pts) =>
