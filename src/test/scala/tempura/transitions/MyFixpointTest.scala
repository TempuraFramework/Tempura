...
import tempura.expression.Core.SortValue.FiniteUniverseValue
import tempura.expression.Core.boolSort
import tempura.expression.Syntax.*
import tempura.helpers.Utils.unexpected
import tempura.smt.SmtSolver.Result
import tempura.expression.Core
import tempura.helpers.Utils
import tempura.parsing.printers.{DotPrinter, FormulaPrinter}
...
/*
  ...
  test("my test 0") {
    ...
    val counter0 = StateVariable("cnt0", "cnt0_next", 0, timeSort)
    ...
    val graphPrinter =
      DotPrinter.Printer(graph, true,
        ...
        Some(e =>
          s"${(graph.labelOf(e._1, e._3).map(_.toString).getOrElse(Core.mkTrue))}"
        ))
    graphPrinter.visualizeDOT(None, true)
  }
  ...
  test("my test 1") {
    ...
    val counter0 = StateVariable("cnt0", "cnt0_next", 0, timeSort)
    val counter1 = StateVariable("cnt1", "cnt1_next", 0, timeSort)
    val flag = StateVariable("flag", "flag_next", 0, Core.boolSort)

    val queueSort = Core.arraySort(timeSort, Core.boolSort)
    val queueDef = interpEnv |- ("queue", queueSort)
    val queue = StateVariable("queue", "queue_next", 0, queueSort)
